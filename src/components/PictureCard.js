...
const PictureCard = ({
  ...
  <Card style={styles.cardStyle}>
    ...
    {pictures.filter(pic => pic.file.uri === uri)[0].flags.items.length !== 0 &&
      pictures
        .filter(pic => pic.file.uri === uri)[0]
        .flags.items.filter(obj => obj.flagOwnerId === user).length >= 1 && (
        <Text style={[styles.pictureUsername, { padding: 10 }]}>
          Content removed.
        </Text>
      )}
    ...
    {pictures.filter(pic => pic.file.uri === uri)[0].flags.items.length !== 0 &&
      pictures
        .filter(pic => pic.file.uri === uri)[0]
        .flags.items.filter(obj => obj.flagOwnerId === user).length === 0 && (
        <Image style={styles.image} source={{ uri: uri }} />
      )}
    {/* Picture has no flags */}
    {pictures.filter(pic => pic.file.uri === uri)[0].flags.items.length ===
      0 && <Image style={styles.image} source={{ uri: uri }} />}
    <View style={styles.cardFooterStyle}>
      ...
      {pictures.filter(pic => pic.file.uri === uri)[0].likes.items.length !==
        0 &&
        pictures
          .filter(pic => pic.file.uri === uri)[0]
          .likes.items.filter(obj => obj.likeOwnerId === user).length === 1 && (
          <LikePictureButton
            color="#FB7777"
            toggleLikePicture={toggleLikePicture}
            name="md-heart"
            likes={pictures}
            uri={uri}
          />
      ...
      {pictures.filter(pic => pic.file.uri === uri)[0].likes.items.length !==
        ...
        pictures
          .filter(pic => pic.file.uri === uri)[0]
          .likes.items.filter(obj => obj.likeOwnerId === user).length === 0 && (
          <LikePictureButton
            color="#FB7777"
            toggleLikePicture={toggleLikePicture}
            name="md-heart-empty"
