...
class Feed extends React.Component {
  ...
  */
  ...
  uploadToS3AndRecordInDynamodb = async uri => {
    ...
    RNS3.put(picture, config).then(response => {
  ...
  */
    ...
    if (Platform.OS === "ios") {
      ActionSheetIOS.showActionSheetWithOptions(
        ...
        buttonIndex => {
          ...
          if (
            buttonIndex === 2 &&
            this.state.pictures.filter(pic => pic.file.uri === uri)[0]
              .pictureOwnerId === this.state.pictureOwnerId
          ) {
  ...
  createFlagPicture = async uri => {
    ...
    const pictureObject = await this.state.pictures.filter(
      photo => photo.file.key === key
    );
